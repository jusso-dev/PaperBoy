...
The production image defaults to the web process. Set `PAPERBOY_PROCESS_TYPE=jobs` on the separate jobs application; its entrypoint applies pending database migrations before starting BullMQ. Any other process value fails closed. Keep that jobs service at desired count 1 or more on AWS. It writes a Redis heartbeat every reconcile. If the heartbeat is missing, `/app/logs` warns and owners/admins can click **Send queued now** to deliver through the same SES path; new API and console sends also fall back to the web process so mail does not sit in `queued` because the worker is down.
