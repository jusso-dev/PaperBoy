...
The worker routes test keys only to the isolated test sink and routes live messages by their persisted provider identity. The shared contract supports send, optional batch and scheduling operations, connection tests, provider receipts, and event mapping. SMTP and explicit Cloudflare identities are live now; SES and Azure identities fail explicitly until their dedicated adapters land. The worker core passes one provider-neutral semantic message including verified attachment bytes. SMTP coverage builds MIME from that value, while the Cloudflare compatibility assertion also converts that exact value into Cloudflare Email Sending's structured, unsigned REST payload.
