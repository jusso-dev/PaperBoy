...
[`openapi.yaml`](openapi.yaml) is the linted OpenAPI 3.1 contract for single-message send/detail/events, organization open tracking, its public signed pixel, webhook configuration, and raw-body signature verification. It also records each private operation's first-class MCP equivalent. Validate it with `pnpm openapi:lint`.
